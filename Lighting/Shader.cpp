...
Shader::Shader(const std::string &vertexPath, const std::string &fragmentPath,
  ...
  if (!geometryPath.empty()) {
    gShaderPath = geometryPath;
  }

  init();
  std::cout << vShaderPath << fShaderPath << gShaderPath << std::endl;
}
...
void Shader::init() {
  ...
  bool hasGShader = false;
  if (!gShaderPath.empty()) {
    gShader = createShader(GL_GEOMETRY_SHADER, gShaderPath);
    hasGShader = true;
  }
...
void Shader::setUbo(const std::string &name, int value) const {
